...
# MAGIC %md
# MAGIC # Function to call Genie Space 

# COMMAND ----------

# MAGIC %sql
# MAGIC USE CATALOG mzervou;
# MAGIC USE SCHEMA media;
...
# MAGIC $$;
# MAGIC
# MAGIC CREATE OR REPLACE FUNCTION ask_genie_media(question STRING COMMENT "The question to ask about the media dataset", contextual_history STRING COMMENT "provide relevant history to be able to answer this question, assume genie doesn\'t keep track of history. Use \'no relevant history\' if there is nothing relevant to answer the question.")
# MAGIC RETURNS STRING
# MAGIC LANGUAGE SQL
# MAGIC COMMENT 'This Agent interacts with the Genie space API to provide answers to questions about the media dataset.'  
# MAGIC RETURN SELECT _genie_query(
# MAGIC   "https://e2-demo-field-eng.cloud.databricks.com/",
# MAGIC   secret('mz-llm-scope', 'media'),
# MAGIC   '01efd80d29511dfdb8cdbcad56e36ddf',  -- genie space id
...
# MAGIC %sql
# MAGIC SELECT * 
# MAGIC FROM vector_search(index => "mzervou.media.generated_reviews_index", query => 'What do people say about blast from the past ', num_results => 5);
...
# MAGIC USE CATALOG mzervou;
# MAGIC USE SCHEMA media;
# MAGIC CREATE OR REPLACE FUNCTION query_reviews(question STRING COMMENT "The question to ask about the reviews for a movie")
# MAGIC RETURNS STRING
# MAGIC LANGUAGE SQL
# MAGIC COMMENT 'movies reviews' 
# MAGIC RETURN SELECT string(collect_set(generated_review)) from vector_search(index => "mzervou.media.generated_reviews_index", query => question, num_results => 1);

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * 
# MAGIC FROM vector_search(index => "mzervou.media.emails_reports_index", query => 'What do people say about Comedy films', num_results => 1);
...
# MAGIC USE CATALOG mzervou;
# MAGIC USE SCHEMA media;
# MAGIC CREATE OR REPLACE FUNCTION query_marketing_research(question STRING COMMENT "The question to ask about the marketing research report for a movie genre")
# MAGIC RETURNS STRING
# MAGIC LANGUAGE SQL
# MAGIC COMMENT 'marketing research report for a movie genre' 
# MAGIC RETURN SELECT string(collect_set(_3)) from vector_search(index => "mzervou.media.emails_reports_index", query => question, num_results => 1);
